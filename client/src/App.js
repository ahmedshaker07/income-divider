...
function App() {
  ...
  React.useEffect(() => {
    ...
    process.env.NODE_ENV === "development"
      ...
      : (url = "https://income-divider.herokuapp.com/api/categories");
    process.env.NODE_ENV === "development"
      ? (url2 = "http://localhost:5000/api/items")
      : (url2 = "https://income-divider.herokuapp.com/api/items");
    process.env.NODE_ENV === "development"
      ? (url3 = "http://localhost:5000/api/users/")
      : (url3 = "https://income-divider.herokuapp.com/api/users/");
    axios
      .get(url)
      .then((res) => {
  ...
  const handleSubmitSalary = (e) => {
    let url = "";
    process.env.NODE_ENV === "development"
      ? (url = "http://localhost:5000/api/categories/assign")
  ...
  const handleDeduct = (event) => {
    ...
    let url3 = "";
    process.env.NODE_ENV === "development"
      ? (url = "http://localhost:5000/api/categories/deduct")
      : (url = "https://income-divider.herokuapp.com/api/categories/deduct");
    process.env.NODE_ENV === "development"
      ? (url2 = "http://localhost:5000/api/items/add")
      : (url2 = "https://income-divider.herokuapp.com/api/items/add");
    process.env.NODE_ENV === "development"
      ? (url3 = "http://localhost:5000/api/users/spentThisMonth")
      : (url3 =
          "https://income-divider.herokuapp.com/api/users/spentThisMonth");
    axios
      .put(url, {
  ...
  const handleAdd = (event) => {
    ...
    // let url3= ""
    process.env.NODE_ENV === "development"
      ? (url = "http://localhost:5000/api/categories/addValue")
      : (url = "https://income-divider.herokuapp.com/api/categories/addValue");
  ...
  const handleUpdate = (e) => {
    ...
    let url = "";
    process.env.NODE_ENV === "development"
      ? (url = "http://localhost:5000/api/categories/edit")
      : (url = "https://income-divider.herokuapp.com/api/categories/edit");
    axios
      .put(url, {
        id: editMode,
  ...
  const handleAddCategory = (e) => {
    ...
    process.env.NODE_ENV === "development"
      ...
      : (url = "https://income-divider.herokuapp.com/api/categories/add");
    axios
      .post(url, {
  ...
  return (
    ...
    >
        ...
        {categories.map((category, index) => {
          ...
          return category._id === editMode ? (
            ...
            >
              ...
              >
                ...
                <input
                  required
                  id="amount"
                  defaultValue={Math.round(category.amount)}
                />
              </div>
              <div
                style={{
            ...
            <div className="category" key={index}>
              <p>
                Name: {category.name} || Percentage {category.value} % || Amount{" "}
                {Math.round(category.amount)} EGP
              </p>
              ...
              <form
                hidden={category.name !== "Others"}
                onSubmit={handleDeductSpecial}
              >
                <input
                  id="specialDeductionValue"
                  type="number"
                  required
                  placeholder="Special Deduction Value"
                />
                <input
                  id="specialDeductionValueDesc"
                  required
                  placeholder="Describtion"
                />
                <button
      ...
      >
        ...
        <small>Total Earnings = {totalMoney}</small>
        <small>Max Buy Now = {buyNow}</small>
        <small>Available This Month = {availableMonthlyBalance}</small>
        <small>Spent This Month = {spentThisMonth}</small>
        <small>Spent Last Month = {spentLastMonth}</small>
        <div>
          {!addCategory ? (
            <button
              onClick={() => {
